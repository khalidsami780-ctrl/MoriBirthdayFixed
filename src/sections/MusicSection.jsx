      ...
      <motion.div style={VS.disc}
        ...
        {[70,54,38].map((sz,i) => (
          <div key={i} style={{ position:'absolute', width:`${sz}%`, height:`${sz}%`, borderRadius:'50%',
            border:`1px solid rgba(255,255,255,${0.04+i*0.025})`, top:'50%', left:'50%', transform:'translate(-50%,-50%)' }} />
        ))}
...
const VS = {
  ...
  disc:  { position:'absolute', inset:0, borderRadius:'50%',
           background:'radial-gradient(circle at 30% 30%, #1a2a4a, #040d1e)',
           border:'1px solid rgba(90,150,240,0.18)',
           display:'flex', alignItems:'center', justifyContent:'center',
           boxShadow:'0 8px 30px rgba(0,0,0,0.7)' },
  label: { position:'absolute', width:'34%', height:'34%', borderRadius:'50%',
           display:'flex', alignItems:'center', justifyContent:'center',
           border:'1px solid rgba(255,255,255,0.07)' },
  dot:   { width:5, height:5, borderRadius:'50%', background:'rgba(255,255,255,0.25)' },
  icon:  { position:'absolute', inset:0, borderRadius:'50%', background:'rgba(0,0,0,0.22)',
           display:'flex', alignItems:'center', justifyContent:'center' },
}
...
/* ─── Song row ───────────────────────────────────────────────── */
function Row({ song, index, active, playing, onSelect }) {
  return (
    <motion.button
      style={{ ...RS.card,
        background: active ? `linear-gradient(135deg,rgba(${hexRgb(song.color)},0.15),rgba(${hexRgb(song.color)},0.05))` : 'rgba(10,22,52,0.42)',
      ...
      <div style={RS.num}>
        {active && playing ? (
          <div style={{ display:'flex', alignItems:'flex-end', gap:2, height:13 }}>
            {[0,1,2].map(i => (
              <motion.div key={i} style={{ width:2.5, height:'100%', background:song.color, borderRadius:2, transformOrigin:'bottom' }}
      ...
      {/* Mood */}
      <span style={{ padding:'2px 9px', borderRadius:50, border:'1px solid', fontSize:'0.63rem', fontFamily:'var(--font-arabic)', direction:'rtl', whiteSpace:'nowrap', flexShrink:0, borderColor:`${song.color}44`, color: active ? song.color : 'rgba(168,200,248,0.3)' }}>
        {song.mood}
      </span>
    </motion.button>
  )
...
export default function MusicSection({ sectionRef }) {
  ...
  return (
    <section ref={sectionRef} id="music" className="section music-section">
      ...
      <Stars count={50} />
      <div className="orb orb-blue" style={{ width:380, height:380, top:'-8%', right:'-8%', opacity:0.1 }} aria-hidden="true" />
      <div className="orb orb-blue" style={{ width:260, height:260, bottom:'4%', left:'-6%', opacity:0.08 }} aria-hidden="true" />
      ...
      >
        {/* Header */}
        <div style={S.header}>
          <p style={S.eyebrow}>أغانينا المفضلة</p>
          <h2 className="t-display" style={S.title}>Our Music</h2>
          <div className="divider" />
          <p className="t-italic" style={S.subtitle}>The songs that hold our memories</p>
        </div>
        ...
        <div className="glass-card" style={S.player}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <AnimatePresence mode="wait">
                <motion.div key={song.id}
                  ...
                  {song.artist !== '—' && <p style={{ fontFamily:'var(--font-body)', fontStyle:'italic', fontSize:'0.76rem', color:'var(--cream-dim)', opacity:0.45, marginTop:2 }}>{song.artist}</p>}
          ...
          <div style={S.controls}>
            <motion.button style={{ ...S.ctrl, opacity:idx===0&&cur<1?0.28:0.75 }} onClick={prev} whileHover={{scale:1.12}} whileTap={{scale:0.9}} aria-label="Previous">
            ...
            <motion.button
              style={{ ...S.playBtn, background:`radial-gradient(circle at 35% 35%,${song.color}44,${song.color}18)`, borderColor:`${song.color}77` }}
              onClick={toggle} whileHover={{scale:1.08,boxShadow:`0 0 28px ${song.color}55`}} whileTap={{scale:0.93}} aria-label={playing?'Pause':'Play'}>
              <AnimatePresence mode="wait">
            ...
            <motion.button style={{ ...S.ctrl, opacity:idx===PLAYLIST.length-1?0.28:0.75 }} onClick={next} disabled={idx===PLAYLIST.length-1} whileHover={{scale:1.12}} whileTap={{scale:0.9}} aria-label="Next">
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'0.65rem' }}>
            <button style={S.muteBtn} onClick={()=>setMuted(m=>!m)}>
              {muted||vol===0
                ...
                : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="11,5 6,9 2,9 2,15 6,15 11,19"/><path d="M15.54,8.46a5,5,0,0,1,0,7.07"/><path d="M19.07,4.93a10,10,0,0,1,0,14.14"/></svg>}
            </button>
        ...
        <div style={{ width:'100%' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'0.4rem' }}>
            {PLAYLIST.map((s,i) => (
              <Row key={s.id} song={s} index={i} active={s.id===song.id} playing={playing&&s.id===song.id} onSelect={selectSong} />
            ))}
          </div>
        </div>
...
const S = {
  ...
  eyebrow:{ fontFamily:'var(--font-arabic)', direction:'rtl', fontSize:'clamp(0.82rem,2.5vw,0.96rem)', color:'var(--blue-pale)', opacity:0.6, marginBottom:'0.5rem' },
  title:  { fontSize:'clamp(1.9rem,7vw,3rem)', background:'linear-gradient(135deg,var(--cream) 0%,var(--blue-pale) 60%,var(--gold-light) 100%)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' },
  subtitle:{ fontSize:'clamp(0.78rem,2.2vw,0.9rem)', color:'var(--cream-dim)', opacity:0.5, marginTop:'0.25rem', letterSpacing:'0.04em' },
  ...
  controls:{ display:'flex', alignItems:'center', justifyContent:'center', gap:'1.1rem' },
