(() => {
  ...
  const sendRouteChange = (nextRoute) => {
    ...
    touchActivity(`Entered ${pageName(nextRoute)}`)
  ...
  const heartbeat = () => {
    if (session.ended) return
    if (Date.now() - session.lastActivityAt >= INACTIVITY_MS) finish('inactive')
  }
